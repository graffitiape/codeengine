 ...
 */
export const createCodeEngineWebFileContentMapper = (extensionsRoot: string, product: typeof import('../product.json')) => {
...
const bundleCodeEngineWebTask = task.define('bundle-vscode-web-OLD', task.series(
	...
	optimize.bundleTask(
		{
			...
			esm: {
				...
				fileContentMapper: createCodeEngineWebFileContentMapper('.build/web/extensions', product)
			}
		}
	)
));

const minifyCodeEngineWebTask = task.define('minify-vscode-web-OLD', task.series(
	bundleCodeEngineWebTask,
	util.rimraf('out-vscode-web-min'),
	optimize.minifyTask('out-vscode-web', `https://main.vscode-cdn.net/sourcemaps/${commit}/core`)
));
gulp.task(minifyCodeEngineWebTask);

// esbuild-based tasks (new)
const sourceMappingURLBase = `https://main.vscode-cdn.net/sourcemaps/${commit}`;
const esbuildBundleCodeEngineWebTask = task.define('esbuild-vscode-web', () => runEsbuildBundle('out-vscode-web', false, true));
const esbuildBundleCodeEngineWebMinTask = task.define('esbuild-vscode-web-min', () => runEsbuildBundle('out-vscode-web-min', true, true, `${sourceMappingURLBase}/core`));
...
['', 'min'].forEach(minified => {
	...
	const vscodeWebTaskCI = task.define(`vscode-web${dashed(minified)}-ci`, task.series(
		...
		minified ? esbuildBundleCodeEngineWebMinTask : esbuildBundleCodeEngineWebTask,
