---
name: add-policy
description: Use when adding, modifying, or reviewing Code Engine configuration policies. Covers the full policy lifecycle from registration to export to platform-specific artifacts. Run on ANY change that adds a `policy:` field to a configuration property.
---
...
Check the `Code Engine - Build` watch task output, or run:
