...
vs/workbench     ← Standard Code Engine workbench
...
### 2.3 How It Differs from Code Engine

| Aspect | Code Engine Workbench | Agent Sessions Window |
|--------|-------------------|----------------------|
...
Views and contributions that should only appear in the agent sessions window (not in regular Code Engine) use `WindowVisibility.Sessions` in their registration.
...
2. Lean on existing Code Engine services (`IPromptsService`, `IMcpService`, `IChatService`)
3. Browser compatibility required — no Node.js APIs
4. Active worktree comes from `IActiveSessionService`

### 10.5 Validation

1. Check `Code Engine - Build` task output for compilation errors before declaring work complete
