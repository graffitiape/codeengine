...
- Open `/extensions/css-language-features/` as the workspace in Code Engine
- In `/extensions/css-language-features/` run `npm run compile`(or `npm run watch`) to build the client and server
- Run the [`Launch Extension`](https://github.com/graffitiape/codeengine/blob/master/extensions/css-language-features/.vscode/launch.json) debug target in the Debug View. This will:
	- Launch a new Code Engine instance with the `css-language-features` extension loaded
...
- Debug the language server process by using `Attach to Node Process` command in the  Code Engine window opened on `css-language-features`.
...
[graffitiape/codeengine-css-languageservice](https://github.com/graffitiape/codeengine-css-languageservice) contains the language smarts for CSS/SCSS/Less.
This extension wraps the css language service into a Language Server for Code Engine.
If you want to fix CSS/SCSS/Less issues or make improvements, you should make changes at [graffitiape/codeengine-css-languageservice](https://github.com/graffitiape/codeengine-css-languageservice).
...
- Clone [graffitiape/codeengine-css-languageservice](https://github.com/graffitiape/codeengine-css-languageservice)
...
- Open both `vscode-css-languageservice` and this extension in a single workspace with [multi-root workspace](https://github.com/graffitiape/codeengine/docs/editor/multi-root-workspaces) feature
