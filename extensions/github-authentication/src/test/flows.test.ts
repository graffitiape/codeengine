...
suite('getFlows', () => {
	...
	const testCases: Array<{ label: string; query: IFlowQuery; expectedFlows: Flows[] }> = [
		{
			label: 'Code Engine Desktop. Local filesystem. GitHub.com',
			query: {
		...
		{
			label: 'Code Engine Desktop. Local filesystem. GitHub Hosted Enterprise',
			query: {
				extensionHost: ExtensionHost.Local,
				isSupportedClient: true,
		...
		{
			label: 'Code Engine Desktop. Local filesystem. GitHub Enterprise Server',
			query: {
				extensionHost: ExtensionHost.Local,
				isSupportedClient: true,
		...
		{
			label: 'Code Engine. Local filesystem. GitHub.com',
			query: {
				extensionHost: ExtensionHost.Local,
		...
		{
			label: 'Code Engine. Local filesystem. GitHub Hosted Enterprise',
			query: {
				extensionHost: ExtensionHost.Local,
				isSupportedClient: false,
				target: GitHubTarget.HostedEnterprise
		...
		{
			label: 'Code Engine. Local filesystem. GitHub Enterprise Server',
			query: {
				extensionHost: ExtensionHost.Local,
				isSupportedClient: false,
				target: GitHubTarget.Enterprise
	...
	suite('preferDeviceCodeFlow configuration', () => {
		...
		test('returns device code flow first when preferDeviceCodeFlow is true - Code Engine Desktop', async () => {
			const config = vscode.workspace.getConfiguration('github-authentication');
