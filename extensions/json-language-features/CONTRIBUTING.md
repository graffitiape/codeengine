...
- Open `/extensions/json-language-features/` as the workspace in Code Engine
- In `/extensions/json-language-features/` run `npm run compile`(or `npm run watch`) to build the client and server
- Run the [`Launch Extension`](https://github.com/graffitiape/codeengine/blob/master/extensions/json-language-features/.vscode/launch.json) debug target in the Debug View. This will:
	- Launch a new Code Engine instance with the `json-language-features` extension loaded
...
- Debug the language server process by using `Attach to Node Process` command in the  Code Engine window opened on `json-language-features`.
...
[graffitiape/codeengine-json-languageservice](https://github.com/graffitiape/codeengine-json-languageservice) is the library that implements the language smarts for JSON.
The JSON language server forwards most the of requests to the service library.
If you want to fix JSON issues or make improvements, you should make changes at [graffitiape/codeengine-json-languageservice](https://github.com/graffitiape/codeengine-json-languageservice).
...
- Clone [graffitiape/codeengine-json-languageservice](https://github.com/graffitiape/codeengine-json-languageservice)
...
- Open both `vscode-json-languageservice` and this extension in two windows or with a single window with the[multi-root workspace](https://github.com/graffitiape/codeengine/docs/editor/multi-root-workspaces) feature.
