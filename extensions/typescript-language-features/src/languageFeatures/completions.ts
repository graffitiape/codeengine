...
class MyCompletionItem extends vscode.CompletionItem {
	...
	constructor(
		...
		if (tsEntry.source && tsEntry.hasAction && client.apiVersion.lt(API.v490)) {
			...
			// https://github.com/graffitiape/codeengine/issues/40311
		...
		const promise = (async (): Promise<ResolvedCompletionItem | undefined> => {
			...
			if (this.useCodeSnippet) {
				...
				if (shouldCompleteFunction) {
					...
					if (parameterCount > 0) {
						//Fix for https://github.com/graffitiape/codeengine/issues/104059
	...
	private async isValidFunctionCompletionContext(
		...
		// https://github.com/graffitiape/codeengine/issues/18131
