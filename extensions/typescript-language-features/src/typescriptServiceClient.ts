...
import { TelemetryReporter, CodeEngineTelemetryReporter } from './logging/telemetry';
...
export default class TypeScriptServiceClient extends Disposable implements ITypeScriptServiceClient {
	...
	constructor(
		...
		this.telemetryReporter = new CodeEngineTelemetryReporter(services.experimentTelemetryReporter, () => {
			if (this.serverState.type === ServerState.Type.Running) {
	...
	private serviceExited(restart: boolean, tsVersion: API): void {
		...
		if (restart) {
			...
			} else if (['vscode-insiders', 'code-engine'].includes(vscode.env.uriScheme)) {
			...
			prompt?.then(async item => {
				...
				if (item === reportIssueItem) {
					...
					// Don't allow reporting issues with old TS versions
						...
						vscode.env.openExternal(vscode.Uri.parse('https://github.com/graffitiape/codeengine/wiki/TypeScript-Issues'));
					}
				}
			});
