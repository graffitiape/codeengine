...
export function getCodeEngineCellLanguageId(metadata: CellMetadata): string | undefined {
	return metadata.metadata?.vscode?.languageId;
}
export function setCodeEngineCellLanguageId(metadata: CellMetadata, languageId: string) {
	metadata.metadata = metadata.metadata || {};
	metadata.metadata.vscode = { languageId };
}
export function removeCodeEngineCellLanguageId(metadata: CellMetadata) {
...
function createCodeCellFromNotebookCell(cell: NotebookCellData, preferredLanguage: string | undefined): nbformat.ICodeCell {
	...
	if (cell.languageId !== preferredLanguage) {
		setCodeEngineCellLanguageId(cellMetadata, cell.languageId);
	} else {
		...
		removeCodeEngineCellLanguageId(cellMetadata);
	}
...
function translateCellErrorOutput(output: NotebookCellOutput): nbformat.IError {
	...
	return {
		...
		// Code Engine needs an `Error` object which requires a `stack` property as a string.
...
function convertOutputMimeToJupyterOutput(mime: string, value: Uint8Array) {
	...
	try {
		if (mime === CellOutputMimeTypes.error) {
			...
			// Code Engine expects bytes when rendering images.
