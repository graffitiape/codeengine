...
export function applyCoreTools(server: McpServer, appService: ApplicationService): RegisteredTool[] {
	...
	tools.push(server.tool(
		...
		'Restart Code Engine with optional workspace or folder and extra command-line arguments',
		{
		...
		async ({ workspaceOrFolder, extraArgs }) => {
			...
			return {
				content: [{
					type: 'text' as const,
					text: `Code Engine restarted successfully${workspaceText}${argsText}`
				}]
			};
		}
	));
	...
	tools.push(server.tool(
		...
		'Stop the Code Engine application',
		async () => {
			const app = await appService.getOrCreateApplication();
			...
			return {
				content: [{
					type: 'text' as const,
					text: 'Code Engine stopped successfully'
				}]
			};
		}
	));
	...
	// 	'Get the quality/build type of Code Engine (Dev, Insiders, Stable, etc.)',
	// 	async () => {
	// 		const info = {
	// 			quality: app.quality,
	// 			remote: app.remote,
	...
	// 	'Send a keybinding to Code Engine (e.g., ctrl+shift+p, cmd+s)',
	// 	{
