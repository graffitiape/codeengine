...
suite('Processes', () => {
	test('buffered sending - simple data', function (done: () => void) {
		if (process.env['CODEENGINE_PID']) {
			return done(); // this test fails when run from within Code Engine
		}
	...
	(!platform.isWindows || process.env['CODEENGINE_PID'] ? test.skip : test)('buffered sending - lots of data (potential deadlock on win32)', function (done: () => void) { // test is only relevant for Windows and seems to crash randomly on some Linux builds
