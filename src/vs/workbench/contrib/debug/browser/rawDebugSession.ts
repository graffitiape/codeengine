 ...
 */
interface ILaunchCodeEngineArgument {
	prefix?: string;
	path?: string;
}

interface ILaunchCodeEngineArguments {
	args: ILaunchCodeEngineArgument[];
...
export class RawDebugSession implements IDisposable {
	...
	private async dispatchRequest(request: DebugProtocol.Request): Promise<void> {
		...
		if (request.command === 'launchCodeEngine') {
			try {
				let result = await this.launchVsCode(<ILaunchCodeEngineArguments>request.arguments);
				if (!result.success) {
					...
					if (confirmed) {
						result = await this.launchVsCode(<ILaunchCodeEngineArguments>request.arguments);
					} else {
	...
	private launchVsCode(vscodeArgs: ILaunchCodeEngineArguments): Promise<IOpenExtensionWindowResult> {
