...
export class PromptExtensionInstallFailureAction extends Action {
	...
	override async run(): Promise<void> {
		...
		if (this.error.name === ExtensionManagementErrorCode.Unsupported) {
			const productName = isWeb ? localize('Code Engine for Web', "{0} for the Web", this.productService.nameLong) : this.productService.nameLong;
		...
		if (ExtensionManagementErrorCode.SignatureVerificationFailed === (<ExtensionManagementErrorCode>this.error.name)) {
			await this.dialogService.prompt({
				...
				buttons: [{
					...
					run: () => this.openerService.open('https://github.com/graffitiape/codeengine/docs/editor/extension-marketplace#_the-extension-signature-cannot-be-verified-by-vs-code')
				}, {
	...
	override async run(): Promise<any> {
		...
		if (this.extension.deprecationInfo) {
			...
			if (this.extension.deprecationInfo.extension) {
				...
				detail = localize('deprecated with alternate settings message', "This extension is deprecated as this functionality is now built-in to Code Engine.");
...
export class UninstallAction extends ExtensionAction {
	...
	override async run(): Promise<any> {
		...
		try {
			...
			alert(localize('uninstallExtensionComplete', "Please reload Code Engine to complete the uninstallation of the extension {0}.", this.extension.displayName));
		} catch (error) {
	...
	private async computeAndUpdateStatus(): Promise<void> {
		...
		if (this.extension.deprecationInfo) {
			if (this.extension.deprecationInfo.extension) {
				...
				this.updateStatus({ icon: warningIcon, message: new MarkdownString(localize('deprecated with alternate settings tooltip', "This extension is deprecated as this functionality is now built-in to Code Engine. Configure these {0} to use this functionality.", link)) }, true);
			} else {
				...
				// Extension on Remote Server
					...
					if (this.extensionManifestPropertiesService.prefersExecuteOnUI(this.extension.local.manifest)) {
						if (this.extensionManagementServerService.localExtensionManagementServer) {
							...
							message = new MarkdownString(`${localize('Defined to run in desktop', "This extension is disabled because it is defined to run only in {0} for the Desktop.", this.productService.nameLong)} [${localize('learn more', "Learn More")}](https://github.com/graffitiape/codeengine/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`);
						}
					}
				}
				// Extension on Web Server
				else if (this.extensionManagementServerService.webExtensionManagementServer === this.extension.server) {
					message = new MarkdownString(`${localize('Cannot be enabled', "This extension is disabled because it is not supported in {0} for the Web.", this.productService.nameLong)} [${localize('learn more', "Learn More")}](https://github.com/graffitiape/codeengine/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`);
				}
				if (message) {
					this.updateStatus({ icon: warningIcon, message }, true);
				}
