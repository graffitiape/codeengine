...
export class ReleaseNotesManager extends Disposable {
	...
	private async getBase(useCurrentFile: boolean) {
		...
		return URI.parse('https://github.com/graffitiape/codeengine/raw');
	}
	...
	private async loadReleaseNotes(version: string, useCurrentFile: boolean): Promise<string> {
		...
		const baseUrl = 'https://github.com/graffitiape/codeengine/raw';
	...
	private async addGAParameters(uri: URI, origin: string, experiment = '1'): Promise<URI> {
		if (supportsTelemetry(this._productService, this._environmentService) && getTelemetryLevel(this._configurationService) === TelemetryLevel.USAGE) {
			if (uri.scheme === 'https' && uri.authority === 'github.com/graffitiape/codeengine') {
	...
	private async renderBody(fileContent: { text: string; base: URI }) {
		...
		<html>
			<head>
				...
				<meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src https: data:; media-src https:; style-src 'nonce-${nonce}' https://github.com/graffitiape/codeengine; script-src 'nonce-${nonce}';">
				<style nonce="${nonce}">
 ...
 * - `IN_PRODUCT` - Content shown in Code Engine (both Stable and Insiders)
 * - `WEB` - Content shown on the website only
 * - `STABLE` - Content shown in Code Engine Stable only
 * - `INSIDERS` - Content shown in Code Engine Insiders only
 *
