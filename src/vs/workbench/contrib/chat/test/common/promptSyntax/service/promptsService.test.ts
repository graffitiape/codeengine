	...
	suite('getCustomAgents', () => {
		...
		test('header with target', async () => {
			...
			await mockFiles(fileService, [
				...
				},
				{
					...
					contents: [
						'---',
						'description: \'Code Engine specialized agent.\'',
						'target: \'vscode\'',
						'model: \'gpt-4\'',
						'---',
						'I am specialized for Code Engine editor tasks.',
					]
				},
				{
			...
			const expected: ICustomAgent[] = [
				...
				},
				{
					name: 'vscode-agent',
					description: 'Code Engine specialized agent.',
					target: Target.CodeEngine,
					model: ['gpt-4'],
					agentInstructions: {
						content: 'I am specialized for Code Engine editor tasks.',
						toolReferences: [],
						metadata: undefined
					},
					handOffs: undefined,
		...
		test('claude agent maps tools and model to vscode equivalents', async () => {
			...
			assert.deepEqual(
				result,
				expected,
				'Claude tools and models must be mapped to Code Engine equivalents; non-Claude agents must remain unchanged.',
			);
		});
