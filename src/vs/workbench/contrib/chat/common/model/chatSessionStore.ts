					...
					try {
						...
						if (!this._didReportIssue) {
							...
							this.dialogService.prompt({
								...
								message: localize('chatSessionStore.writeError', 'Error serializing chat session for storage. The session will be lost if the window is closed. Please report this issue to the Code Engine team:\n\n{0}', e.stack || toErrorMessage(e)),
								buttons: [
									{ label: localize('reportIssue', 'Report Issue'), run: () => this.openerService.open('https://github.com/graffitiape/codeengine/issues/new?template=bug_report.md') }
								]
							});
						}
...
// Don't throw away index when moving backwards in Code Engine version. Try to recover it. But this scenario is hard.
