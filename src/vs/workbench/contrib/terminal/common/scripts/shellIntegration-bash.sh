...
if [[ -n "${CODEENGINE_SHELL_INTEGRATION:-}" ]]; then
	builtin return
fi

CODEENGINE_SHELL_INTEGRATION=1
...
__vscode_shell_env_reporting="${CODEENGINE_SHELL_ENV_REPORTING:-}"
unset CODEENGINE_SHELL_ENV_REPORTING
...
if [ "$CODEENGINE_INJECTION" == "1" ]; then
	if [ -z "$CODEENGINE_SHELL_LOGIN" ]; then
		if [ -r ~/.bashrc ]; then
			. ~/.bashrc
		fi
	else
		...
		builtin unset CODEENGINE_SHELL_LOGIN

		# Apply any explicit path prefix (see #99878)
		if [ -n "${CODEENGINE_PATH_PREFIX:-}" ]; then
			export PATH="$CODEENGINE_PATH_PREFIX$PATH"
			builtin unset CODEENGINE_PATH_PREFIX
		fi
	fi
	builtin unset CODEENGINE_INJECTION
fi

if [ -z "$CODEENGINE_SHELL_INTEGRATION" ]; then
	builtin return
fi

# Prevent AI-executed commands from polluting shell history
if [ "${CODEENGINE_PREVENT_SHELL_HISTORY:-}" = "1" ]; then
	export HISTCONTROL="ignorespace"
	builtin unset CODEENGINE_PREVENT_SHELL_HISTORY
fi

# Apply EnvironmentVariableCollections if needed
if [ -n "${CODEENGINE_ENV_REPLACE:-}" ]; then
	IFS=':' read -ra ADDR <<< "$CODEENGINE_ENV_REPLACE"
	for ITEM in "${ADDR[@]}"; do
		VARNAME="$(echo $ITEM | cut -d "=" -f 1)"
		VALUE="$(echo -e "$ITEM" | cut -d "=" -f 2-)"
		export $VARNAME="$VALUE"
	done
	builtin unset CODEENGINE_ENV_REPLACE
fi
if [ -n "${CODEENGINE_ENV_PREPEND:-}" ]; then
	IFS=':' read -ra ADDR <<< "$CODEENGINE_ENV_PREPEND"
	for ITEM in "${ADDR[@]}"; do
		VARNAME="$(echo $ITEM | cut -d "=" -f 1)"
		VALUE="$(echo -e "$ITEM" | cut -d "=" -f 2-)"
		export $VARNAME="$VALUE${!VARNAME}"
	done
	builtin unset CODEENGINE_ENV_PREPEND
fi
if [ -n "${CODEENGINE_ENV_APPEND:-}" ]; then
	IFS=':' read -ra ADDR <<< "$CODEENGINE_ENV_APPEND"
	for ITEM in "${ADDR[@]}"; do
		VARNAME="$(echo $ITEM | cut -d "=" -f 1)"
		VALUE="$(echo -e "$ITEM" | cut -d "=" -f 2-)"
		export $VARNAME="${!VARNAME}$VALUE"
	done
	builtin unset CODEENGINE_ENV_APPEND
fi

# Register Python shell activate hooks
# Prevent multiple activation with guard
if [ -z "${CODEENGINE_PYTHON_AUTOACTIVATE_GUARD:-}" ]; then
	export CODEENGINE_PYTHON_AUTOACTIVATE_GUARD=1
	if [ -n "${CODEENGINE_PYTHON_BASH_ACTIVATE:-}" ] && [ "$TERM_PROGRAM" = "vscode" ]; then
		# Prevent crashing by negating exit code
		if ! builtin eval "$CODEENGINE_PYTHON_BASH_ACTIVATE"; then
			__vsc_activation_status=$?
			builtin printf '\x1b[0m\x1b[7m * \x1b[0;103m Code Engine Python bash activation failed with exit code %d \x1b[0m' "$__vsc_activation_status"
		fi
	fi
	# Remove any leftover Python activation env vars.
	for var in "${!CODEENGINE_PYTHON_@}"; do
		case "$var" in
			CODEENGINE_PYTHON_*_ACTIVATE)
				unset "$var"
				;;
		esac
	done
...
__vsc_nonce="$CODEENGINE_NONCE"
unset CODEENGINE_NONCE

# Some features should only work in Insiders
__vsc_stable="$CODEENGINE_STABLE"
unset CODEENGINE_STABLE
