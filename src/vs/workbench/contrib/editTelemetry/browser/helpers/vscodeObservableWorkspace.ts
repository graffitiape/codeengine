...
export class CodeEngineWorkspace extends ObservableWorkspace implements IDisposable {
	...
	constructor(
		...
		const documents = mapObservableArrayCached(this, models, (m, store) => {
			...
			return store.add(new CodeEngineDocument(m));
...
export class CodeEngineDocument extends Disposable implements IObservableDocument {
