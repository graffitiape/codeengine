...
import { CodeEngineWorkspace } from './helpers/vscodeObservableWorkspace.js';
...
export class EditTelemetryContribution extends Disposable {
	constructor(
		...
		const workspace = derived(reader => reader.store.add(instantiationService.createInstance(CodeEngineWorkspace)));
