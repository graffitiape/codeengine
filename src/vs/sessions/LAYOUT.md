...
The Agent Sessions Workbench (`Workbench` in `sessions/browser/workbench.ts`) provides a simplified, fixed layout optimized for agent session workflows. Unlike the default Code Engine workbench, this layout:
...
| Open... | (submenu) | Right toolbar (`TitleBarRight`) | Split button submenu: Open Terminal, Open in Code Engine |
...
- Contains "Open Terminal" (opens terminal at session worktree) and "Open in Code Engine" (opens worktree in new Code Engine window)
...
│   │   ├── chat.contribution.ts            # Open in Code Engine, Open Terminal, branch chat, run script, prompts service
