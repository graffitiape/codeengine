...
export class WindowsStateHandler extends Disposable {
	...
	private registerListeners(): void {
		...
		// shutting down the application (https://github.com/graffitiape/codeengine/issues/87171)
		...
		if (windowConfig?.newWindowDimensions) {
			if (windowConfig.newWindowDimensions === 'maximized') {
				...
				if (lastActiveState.mode === WindowMode.Fullscreen) {
					state.mode = WindowMode.Fullscreen; // only take mode (fixes https://github.com/graffitiape/codeengine/issues/19331)
				} else {
					state = {
						...lastActiveState,
