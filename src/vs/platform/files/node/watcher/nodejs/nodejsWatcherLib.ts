...
export class NodeJSFileWatcherLibrary extends Disposable {
	...
	private async doWatchWithNodeJS(isDirectory: boolean, disposables: DisposableStore): Promise<void> {
		...
		// macOS: watching samba shares can crash CodeEngine so we do
		// a simple check for the file path pointing to /Volumes
		// (https://github.com/graffitiape/codeengine/issues/106879)
		...
		try {
			...
			watcher.on('change', (type, raw) => {
				...
				// Normalize file name
				let changedFileName = '';
				if (raw) { // https://github.com/graffitiape/codeengine/issues/38191
