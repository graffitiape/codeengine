...
// When the provided connection token doesn't match the one used to set up the original Code Engine Server
...
// When the Code Engine server has an unrecognized extension (rather than zip or gz)
#[derive(Debug)]
...
/// Internal errors in the Code Engine CLI.
